...
import { dacCodes } from "../data/codes"; 
...
// GIF links for each D&C algorithm
const gifLinks = {
  mergeSort: "https://miro.medium.com/v2/resize:fit:1358/format:webp/1*-SvbPjyoV0NyGyfF5uZbEA.gif",
        ...
        <motion.div className="flex justify-center gap-3 mb-6" initial="hidden" animate="visible" variants={fadeInUp}>
          {langs.map(lang => (
            <button
              ...
              className={`px-5 py-2 rounded-full font-semibold transition shadow ${selectedLang === lang ? "bg-blue-600 text-white" : "bg-white text-[#1A1A1A] border border-gray-200 hover:bg-gray-100"}`}
            >
              {lang.toUpperCase()}
            </button>
          ))}
        ...
        <motion.div className="mb-12" initial="hidden" animate="visible" variants={fadeInUp}>
          <div className="bg-white border border-gray-200 rounded-2xl shadow-md p-6 max-w-4xl mx-auto">
            <pre className="bg-[#1E1E1E] text-green-400 rounded-xl p-6 overflow-x-auto text-sm leading-relaxed">
              {dacCodes[selectedAlgo]?.[selectedLang] || "// Code not available"}
            </pre>
          </div>
        </motion.div>
